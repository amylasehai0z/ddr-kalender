# 📅 Jahrweiser DDR

> Über **200 Gedenk- und Feiertage der DDR** (Deutsche Demokratische Republik, 1949–1990)
> als fortlaufendes iCalendar-Abonnement und iOS/iPadOS-App

---

## 📱 App

**Jahrweiser DDR** ist als kostenlose App für iPhone und iPad verfügbar:

- **Heute** — tägliche Gedenktage mit ausführlichen Hintergrundinformationen
- **Zeitreise** — alle Termine als blätterbare Karten mit Suche und Kategoriefilter
- **Jahreskalender** — Übersicht aller Monate mit farbigen Markierungen
- **Kalender-Abo** — direkte Einbindung in die iPhone-Kalender-App

**Webseite:** [amylasehai0z.github.io/ddr-kalender](https://amylasehai0z.github.io/ddr-kalender/)

---

## 📅 Kalender abonnieren

Die Kalenderdatei kann direkt in jede gängige Kalender-App als dauerhaftes Abonnement eingebunden werden:

```
https://amylasehai0z.github.io/ddr-kalender/ddr-feiertage.ics
```

| App | Anleitung |
| --- | --------- |
| **Apple Kalender** | Ablage → Neues Kalenderabonnement |
| **Google Calendar** | Einstellungen → Weitere Kalender → Per URL hinzufügen |
| **Outlook** | Kalender hinzufügen → Aus dem Internet abonnieren |

---

## 📋 Inhalt

Der Kalender enthält **über 10.000 Einträge** für die Jahre **2024–2075**.
Jeder Termin enthält eine ausführliche historische Notiz.
Ein **automatischer Hinweis erscheint um 6:00 Uhr morgens**.

### Kategorien

#### 🏛️ Staatsfeiertage & politische Gedenktage
- Liebknecht & Luxemburg Gedenktag (Sonntag vor dem 15. Januar)
- Gründungstag der SED (21. April)
- Geburtstag W. I. Lenin (22. April)
- Tag der Arbeit (1. Mai)
- Jahrestag der Befreiung (8. Mai)
- Tag der Republik (7. Oktober) — Nationalfeiertag der DDR
- Jahrestag der Oktoberrevolution (7. November)
- u. v. m.

#### 👤 Personen — Geburts- und Todestage
Staatsführer, Kulturschaffende, Sportler und Künstler der DDR, darunter:
- Wilhelm Pieck, Walter Ulbricht, Erich Honecker, Egon Krenz
- Karl Marx, Friedrich Engels, Lenin, Ernst Thälmann
- Bertolt Brecht, Hanns Eisler, Anna Seghers, Christa Wolf, Heiner Müller
- Erwin Geschonneck, Manfred Krug, Angelica Domröse, Ulrich Mühe
- Marita Koch, Gerhard Gundermann, Hannes Hegen
- u. v. m. (über 80 Persönlichkeiten)

#### 🌍 Internationale Gedenktage
- Internationaler Frauentag (8. März)
- Internationaler Kindertag (1. Juni)
- Weltfriedenstag (1. September)
- OdF-Tag — Opfer des Faschismus (2. Sonntag im September)
- u. v. m.

#### ⚙️ Berufstage
- Tag des Bergmanns und Energiearbeiters (1. Sonntag im Juli)
- Tag des Eisenbahners (2. Sonntag im Juni)
- Tag des Lehrers (12. Juni)
- Tag des Metallarbeiters, Chemiearbeiters, Bauarbeiters
- u. v. m.

---

## 📁 Dateien

| Datei | Beschreibung |
| ----- | ------------ |
| `ddr-feiertage.ics` | Fertige Kalenderdatei zum Abonnieren (2024–2075) |
| `ddr_events.json` | JSON-Datei für die iOS-App (2024–2035) |
| `generate_ddr_ical.py` | Python-Skript zur Generierung beider Dateien |
| `index.html` | GitHub Pages Webseite |

---

## ⚙️ Selbst generieren

Voraussetzung: Python 3

```bash
git clone https://github.com/amylasehai0z/ddr-kalender
cd ddr-kalender
python3 generate_ddr_ical.py
```

Das Skript generiert automatisch beide Dateien:
- `ddr-feiertage.ics` — für das Kalender-Abonnement
- `ddr_events.json` — für die iOS-App

Der Jahresbereich lässt sich im Skript unter `YEARS = range(2024, 2076)` anpassen.

---

## 📜 Hintergrund

Die DDR pflegte einen eigenen sozialistischen Festkalender, der religiöse Feiertage weitgehend durch politische Gedenktage ersetzte oder ergänzte. Neben den gesetzlichen Feiertagen prägten Kampftage, Berufsehrentage und gesellschaftliche Festtraditionen wie die **Jugendweihe** das öffentliche Leben.

Dieser Kalender dokumentiert diesen Festkalender als lebendiges iCal-Abonnement — nutzbar in jeder modernen Kalender-App, mit Erinnerungen und ausführlichen historischen Notizen zu jedem Termin.

---

## 📬 Kontakt & Impressum

**Timo König** · Steinbecker Softwaredienste
Auf dem Glockenberg 4 · 38707 Clausthal-Zellerfeld
✉️ steinbeckersoftware@proton.me

---

## ⚖️ Lizenz

Historische Daten sind gemeinfrei. Der Code steht unter der [MIT License](LICENSE).
